...
public sealed class TerminalInterface(ManufacturingDbContext context)
{
    ...
    void ListAvailableProducts()
    {
        ...
        {
            Console.WriteLine($"Id: {product.ProductId} | Name: {product.ProductName} | Weight: {product.WeightInKilogramsPerUnit} kg/unit");
        }
    }
    ...
    void CreateNewProject()
    {
        ...
        try
        {
            ...
            Console.WriteLine($"Project created successfully with Id: {newlyCreatedProjectId}");
        }
    ...
    void CreateNewWorkOrder()
    {
        ...
        if (!int.TryParse(Console.ReadLine(), out int selectedProjectId)) return;
        ...
        while (isAddingWorkOrderLines)
        {
            ...
            if (productIdInput?.Equals("done", StringComparison.OrdinalIgnoreCase) == true) break;

            if (!int.TryParse(productIdInput, out int selectedProductId)) continue;
            Console.Write("Enter Quantity: ");
            if (int.TryParse(Console.ReadLine(), out int requestedQuantity))
            {
                workOrderLineRequests.Add(new WorkOrderLineRequest(selectedProductId, requestedQuantity));
            }
        }
        ...
        try
        {
            int newlyCreatedWorkOrderId = _createWorkOrderCommand.Execute(selectedProjectId, workOrderLineRequests);
            Console.WriteLine($"Work Order created successfully with Id: {newlyCreatedWorkOrderId}");
        }
        catch (Exception exception)
        {
            Console.WriteLine($"Error occurred: {exception.Message}");
    ...
    {
        Console.Write("Enter Work Order Id: ");
        ...
        Console.WriteLine($"\nWork Order #{retrievedWorkOrder.WorkOrderId} (Project: {retrievedWorkOrder.Project?.ProjectName}) - Created: {retrievedWorkOrder.CreatedDateTimeUtc:yyyy-MM-dd HH:mm:ss} UTC");
